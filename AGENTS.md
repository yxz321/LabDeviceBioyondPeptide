...
- When editing `BioyondPeptideStation` exposed `@action` inputs, manual-confirm goals, handles, or parameter classes, follow `developer_docs/bioyond_peptide_station_action_args.md`. It defines the required `Args:` display-name format, required `*` marker, user-facing description style, parameter ordering, internal-resolved parameter labels such as `order_id[<order_id>*]`, and the standard `实验编号` wording for `order_code`.
...
`station.py` (`BioyondWorkstation` base + `BioyondResourceSynchronizer`), `bioyond_rpc.py` (`BioyondV1RPC` HTTP client), `debug_call_log.py`, `graphio_bioyond.py` (the two Bioyond resource-conversion functions extracted from unilabos `graphio`), and `workstation_http_service.py` (the LIMS callback HTTP server). These were modified by the peptide work but the published/`dev` `unilabos` still ships the old versions, so they travel with the package. `workstation_http_service.py` specifically is vendored because the installed unilabos copy drops bioyond material-change pushes — its `_handle_material_change_report` bioyond branch acknowledges the POST but returns before delegating, never calling `process_material_change_report`; the vendored copy fixes that (mirroring `_handle_error_handling_report`). **Everything else** (logger, `WorkstationBase`, registry decorators, etc.) is imported from the installed `unilabos`. When a vendored module diverges further from upstream, it stays here; do not assume `unilabos` has the same code.
